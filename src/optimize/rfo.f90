...
!>--- start with a printout of the preceeding single point
    ...
    if (pr) then
      ...
      !> BUGFIX: commas around f7.2 are MANDATORY, see the identical fix in
      !> src/optimize/ancopt.f90. gfortran >=15 raises
      !> "Fortran runtime error: Missing comma between descriptors" at runtime.
