...
subroutine confscript_morehelp(flag)
  ...
  case default
    ...
    write (*,'(5x,''                     for solvent <string>'')')
    write (*,'(5x,''-draco [set]       : charge-dependent (DRACO) cavity radii'')')
    write (*,'(5x,''                     on top of -g/-alpb. Works with every'')')
    write (*,'(5x,''                     method run through the xtb binary:'')')
    write (*,'(5x,''                     GFN1/GFN2/GFN-FF and g-xTB.'')')
    write (*,'(5x,''                     [set]=gsolv,pka,ddg (GFN2 only,'')')
    write (*,'(5x,''                     default ddg); other methods have'')')
    write (*,'(5x,''                     one set and ignore the argument'')')
